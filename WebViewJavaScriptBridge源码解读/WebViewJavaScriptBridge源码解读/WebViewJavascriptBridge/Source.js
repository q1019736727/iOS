...
(function() {
    ...
    function _dispatchMessageFromObjC(messageJSON) {
        ...
        function _doDispatchMessageFromObjC() {
            ...
            //回调
            if (message.responseId) { //JS(callHandler)调用iOS走这里   message.responseId对应存储在responseCallbacks里面的函数id
                responseCallback = responseCallbacks[message.responseId];
                if (!responseCallback) {
                    return;
                }
                responseCallback(message.responseData);//执行函数，回调给JS，里面包含回调的数据responseData
                delete responseCallbacks[message.responseId];
            } else {//iOS主动调用JS(registerHandler)走这里
                if (message.callbackId) {//有 callbackId 说明OC传入了回调
                    var callbackResponseId = message.callbackId;
                    responseCallback = function(responseData) {//以test.html的例子为例这里的responseData就是{'userId': '123456', 'blog': '标哥的技术博客'}
